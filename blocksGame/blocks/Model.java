...
class Model {
        ...
        while (row + r < height()) {
            while (col + c < width()) {
                if (piece.height() + row > height()
                        || piece.width() + col > width()
                        || (piece.get(r, c) && get(row + r, col + c))) {
                    return false;
                }
                c += 1;
            }
    ...
    int[][] rowColumnCounts() {
        ...
        while (r < height()) {
            while (c < width()) {
                ...
                result[1][c] += cCount; c += 1; cCount = 0;
            }
            result[0][r] = rCount; r += 1; rCount = 0; c = 0;
        }
        return result;
    }
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        ...
        clearScore = ((nrows * width()) + (ncols * height()));
        if (_streakLength > 0) {
            clearScore = _streakLength * clearScore;
        }
        return clearScore;
    }
